...
async function main() {
  ...
  // A) Write clients/<slug>/business.js (template uses LNP as starting point)
  const slugForBusiness = (cliArgs.slug || shortName).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  const clientDir = join(ROOT, 'clients', slugForBusiness);
  if (!existsSync(clientDir)) mkdirSync(clientDir, { recursive: true });
  const businessPath = join(clientDir, 'business.js');
  console.log(`  → clients/${slugForBusiness}/business.js`);
  // Seed from la-nona-pato template
  ...
  // B) Write per-client .env (read by vite.config.js based on CLIENT env var)
  // The slug is the directory name under clients/, derived from shortName if not provided.
