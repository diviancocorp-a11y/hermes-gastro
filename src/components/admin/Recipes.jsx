...
function Recipes({recipes,setRecipes,ingredients,calculateRecipeCost,overlay,setOverlay,showToast,loadAll}){
  const [sr,setSr]=useState("");
  const [showArchived,setShowArchived]=useState(false);
  const active=recipes.filter(r=>!r.is_archived);
  const archived=recipes.filter(r=>r.is_archived);
  ...
  return(<>
    ...
    <div className="s">
      ...
      :filt.map(r=>{
        const c=calculateRecipeCost(r);const m=r.sale_price>0?((r.sale_price-c)/r.sale_price*100):0;
        const bc=m>=50?"var(--gn)":m>=30?"var(--yw)":"var(--rd)";
        return(<div key={r.id} className="c" style={{opacity:r.is_archived?0.6:1}} onClick={()=>setOverlay({type:"viewR",data:r})}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            ...
            <div style={{textAlign:"right",display:"flex",alignItems:"center",gap:8}}>
              <div><div style={{fontWeight:700,fontSize:15}}>${formatInt(r.sale_price)}</div><div style={{fontSize:12,color:"var(--t3)"}}>C: ${formatMoney(c)}</div></div>
              <button style={{background:"none",border:"none",cursor:"pointer",padding:4}} onClick={async(e)=>{e.stopPropagation();const nv=r.visible===false;const ok=await toggleRecipeVisibility(r.id,nv);if(ok){setRecipes(p=>p.map(x=>x.id===r.id?{...x,visible:nv}:x));showToast(nv?"Visible en catálogo":"Oculta del catálogo");}else{showToast("Error al cambiar visibilidad");}}}>{r.visible!==false?<span style={{color:"var(--gn)"}}>{Icon.eye({size:14})}</span>:<span style={{color:"var(--t3)"}}>{Icon.eyeOff({size:14})}</span>}</button>
            </div>
          </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
            ...
            <span style={{fontSize:11,color:"var(--gn)",fontWeight:700}}>G: ${formatMoney(r.sale_price-c)}</span>
          </div>
        </div>);
      })}
    </div>
    <button className="fab" onClick={()=>setOverlay({type:"editR",data:null})}>{Icon.plus({size:24,color:"#fff"})}</button>
    {overlay?.type==="viewR"&&<RecDet r={overlay.data} ingredients={ingredients} calculateRecipeCost={calculateRecipeCost} onClose={()=>setOverlay(null)} onEdit={async()=>{
      let d={...overlay.data};
    ...
    onArchive={async(id)=>{await archiveRecipe(id);setRecipes(p=>p.map(x=>x.id===id?{...x,is_archived:true}:x));setOverlay(null);showToast("Receta archivada · El historial se conserva");}}
    onUnarchive={async(id)=>{await unarchiveRecipe(id);setRecipes(p=>p.map(x=>x.id===id?{...x,is_archived:false}:x));setOverlay(null);showToast("Receta restaurada");}}
    />}
    {overlay?.type==="editR"&&<RecForm data={overlay.data} ingredients={ingredients} recipes={recipes} onClose={()=>setOverlay(null)} onSave={async(r)=>{
      ...
      if(saved?.__error==='duplicate'){showToast("⚠️ Ya existe una receta activa con ese nombre");return;}
      if(saved){
        ...
        await loadAll();setOverlay(null);showToast(r.id?"Actualizada":"Creada");
      }
    }}/>}
  </>);
}

function RecDet({r,ingredients,calculateRecipeCost,onClose,onEdit,onArchive,onUnarchive}){
  const c=calculateRecipeCost(r);const m=r.sale_price>0?((r.sale_price-c)/r.sale_price*100):0;
  return(<div className="po"><div className="ph"><button onClick={onClose}>{Icon.back({})}</button><h2>{r.name}</h2>{!r.is_archived&&<button onClick={onEdit}>{Icon.edit({})}</button>}</div><div className="pb">
    ...
    <div className="sg" style={{padding:0,marginBottom:16}}>
      <div className="sc"><div className="sl">Venta</div><div className="sv2">${formatInt(r.sale_price)}</div></div>
      <div className="sc"><div className="sl">Costo</div><div className="sv2 sva">${formatMoney(c)}</div></div>
      <div className="sc"><div className="sl">Ganancia</div><div className="sv2 svg2">${formatMoney(r.sale_price-c)}</div></div>
    ...
    <div className="c" style={{padding:"8px 14px"}}>
      ...
      {(r.ingredients||[]).map((ri,i)=>{const ig=ingredients.find(x=>x.id===ri.ingredient_id);if(!ig)return null;
        return(<div key={i} className="rir"><div className="rin">{ig.name}</div><div className="riq">{ri.quantity} {ig.unit}</div><div className="ric">${formatMoney((ig.cost||0)*ri.quantity)}</div></div>);
      })}
      <div className="rir" style={{fontWeight:700,borderTop:"2px solid var(--b2)"}}><div className="rin">TOTAL</div><div className="riq"></div><div className="ric">${formatMoney(c)}</div></div>
    </div>
    {r.is_archived
      ...
      :(<button className="btn bd" style={{marginTop:16}} onClick={()=>{if(window.confirm(`¿Archivar "${r.name}"? No se eliminará el historial de ventas.`))onArchive(r.id);}}>{Icon.trash({size:16})} Archivar receta</button>)
    }
  </div></div>);
}
...
function RecForm({data,ingredients,recipes,onClose,onSave}){
  ...
  const totalCost=(f.ingredients||[]).reduce((s,ri)=>{const ig=ingredients.find(x=>x.id===ri.ingredient_id);return s+(ig?(ig.cost||0)*(ri.quantity||0):0);},0);
  ...
  return(<div className="po"><div className="ph"><button onClick={onClose}>{Icon.back({})}</button><h2>{data?"Editar":"Nueva"} Receta</h2></div><div className="pb">
      ...
      <div className="img-upload-wrap">
        ...
        <div className="img-upload-row">
          ...
          <button className="btn bs bsm" onClick={()=>fileRef.current?.click()} disabled={uploading} style={{flex:1}}>
            {uploading?"Subiendo...":f.image_url?`${Icon.edit({size:13})} Cambiar foto`:`📷 Subir foto`}
          </button>
    ...
    {f.is_combo&&(<div style={{marginBottom:12}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        ...
        <button className="btn bs bsm" onClick={addComboItem}>{Icon.plus({size:14})} +</button>
      </div>
      <div className="c" style={{padding:"4px 14px"}}>
        ...
        :comboItems.map((ci,i)=>(
          <div key={i} className="fr" style={{alignItems:"center",padding:"4px 0",borderBottom:"1px solid var(--b2)"}}>
            ...
            <button style={{background:"none",border:"none",color:"var(--rd)",cursor:"pointer",marginLeft:4}} onClick={()=>delCombo(i)}>{Icon.trash({size:14})}</button>
          </div>
        ))}
      </div>
    </div>)}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"0 0 8px"}}><label className="fl" style={{margin:0}}>Ingredientes ({(f.ingredients||[]).length})</label><button className="btn bs bsm" onClick={()=>setAd(true)}>{Icon.plus({size:14})} +</button></div>
    {ad&&<div className="c" style={{background:"var(--b2)",marginBottom:10}}>
      <div className="fg"><select className="fin" value={si} onChange={e=>setSi(e.target.value)}><option value="">Seleccionar...</option>{ingredients.map(i=><option key={i.id} value={i.id}>{i.name} ({i.unit})</option>)}</select></div>
      ...
      <button className="btn bp bsm" onClick={addI} style={{alignSelf:"flex-end"}}>{Icon.check({size:16,color:"#fff"})}</button></div>
    </div>}
    <div className="c" style={{padding:"4px 14px"}}>
      {(f.ingredients||[]).length===0?<div className="empty" style={{padding:16}}>Sin ingredientes</div>
      :(f.ingredients||[]).map((ri,i)=>{const ig=ingredients.find(x=>x.id===ri.ingredient_id);
        return(<div key={i} className="rir"><div className="rin">{ig?.name||"?"}</div><div className="riq">{ri.quantity} {ig?.unit||""}</div>
          <button style={{background:"none",border:"none",color:"var(--rd)",cursor:"pointer"}} onClick={()=>s("ingredients",f.ingredients.filter((_,j)=>j!==i))}>{Icon.trash({size:14})}</button></div>);
      })}
    </div>
      ...
      {showRel&&<div className="c" style={{padding:"8px 14px",maxHeight:180,overflowY:"auto"}}>
        ...
        :otherRecs.map(r=>{const sel=(f.related_ids||[]).includes(r.id);
          return(<div key={r.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid var(--b2)",cursor:"pointer"}} onClick={()=>toggleRel(r.id)}>
            ...
            <span style={{fontSize:12,color:"var(--t3)"}}>${formatInt(r.sale_price)}</span>
          </div>);
        })}
      </div>}
    </div>
    {profitNeg&&<div style={{background:"#FFF8E1",color:"#8D6E00",fontSize:13,padding:"10px 14px",borderRadius:10,marginTop:12,display:"flex",gap:8,alignItems:"flex-start"}}>
      ...
      <div><strong>Precio por debajo del costo.</strong> El costo de ingredientes (${formatMoney(totalCost)}) supera el precio de venta (${formatInt(f.sale_price||0)}). Podés guardar igual.</div>
    </div>}
    ...
    <button className="btn bp" style={{marginTop:12,opacity:canSave?1:0.5}} disabled={!canSave} onClick={()=>canSave&&onSave({...f,comboItems})}>{Icon.check({size:18,color:"#fff"})} {data?"Guardar":"Crear"}</button>
  </div></div>);
}
