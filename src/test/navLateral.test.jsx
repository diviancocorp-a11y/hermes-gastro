...
describe('NavLateral — contrato de layout', () => {
  ...
  it('Dico abierto no bloquea la apertura normal de la sidebar', () => {
    const reglas = sidebarCss.replace(/\/\*[\s\S]*?\*\//g, '');
    ...
    for (const bloque of porDico) {
      ...
      expect(cuerpo).not.toMatch(/--ag-sidebar-ancho\s*:/);
      expect(cuerpo).not.toMatch(/--ag-dic-col\s*:/);
      expect(cuerpo).not.toMatch(/\.ag-sidebar-label/);
    }
  });
