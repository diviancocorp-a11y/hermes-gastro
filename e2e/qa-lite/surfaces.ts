...
export async function irAProductos(page: Page) {
  ...
  /* La vista real nace plegada para que 21 productos no se conviertan en una
     pared. Los recorridos de QA que necesitan manipular todas las filas abren
     cada categoria de forma explicita antes de buscarlas. */
  const categorias = page.getByRole('button', { name: /^Abrir categoría / })
  for (let quedan = await categorias.count(); quedan > 0; quedan = await categorias.count()) {
